...
export async function GET(req: NextRequest) {
  ...
  const url = project?.finalVideoUrl;
  if (!url) {
    return NextResponse.json({ ok: false, error: "완성 영상이 없어요" }, { status: 404 });
  }
  const r = await fetch(url);
  if (!r.ok || !r.body) {
    return NextResponse.json({ ok: false, error: "영상을 불러오지 못했어요" }, { status: 502 });
  }
  const filename = `aninews-${projectId.slice(0, 8)}.mp4`;
