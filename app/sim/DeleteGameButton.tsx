...
// 시뮬 게임 카드의 삭제 버튼 — 확인 후 DELETE 호출(플레이 세션도 함께 삭제), 목록 새로고침.
export default function DeleteGameButton({
  ...
  async function onDelete(e: React.MouseEvent) {
    ...
    e.stopPropagation();
    if (busy) return;
    if (!confirm(`"${title || "이 게임"}" 을(를) 삭제할까요? 플레이 기록도 함께 지워지고 되돌릴 수 없어요.`)) return;
    setBusy(true);
    try {
      const res = await fetch(`/api/sim/game/${encodeURIComponent(gameId)}`, {
        method: "DELETE",
      });
