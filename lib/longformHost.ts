...
export async function generateHostScript(args: {
  ...
  // closing 은 정확히 1씬(구독·좋아요).
  if (closing.length === 0) {
    closing = [
      { narration: "재밌으셨다면 구독과 좋아요 눌러주세요!", imagePrompt: "The two host mascots pointing at a red subscribe button, cheering, bright pop background." },
    ];
